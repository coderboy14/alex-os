...
uint32_t ata_read(ata_dev_t * device, uint32_t offset, uint32_t size, char * buf) {
    /*
    ...
    while(counter <= end) {
        ...
        char * ret = ata_read_sector(device, counter);
...
uint32_t ata_write(ata_dev_t * device, uint32_t offset, uint32_t size, char * buf) {
    uint32_t start = offset / SECTOR_SIZE;
    uint32_t start_offset = offset % SECTOR_SIZE;
    ...
    while(counter <= end) {
        ...
        ata_write_sector(device, counter, ret);
